...
- **Razorpay** — [`src/lib/razorpay/`](./src/lib/razorpay/), Standard Checkout (`checkout.js` overlay), fulfilling through the shared [`src/lib/payments/fulfill-order.ts`](./src/lib/payments/fulfill-order.ts) into `orders` / `order_items` / `customers` / `deliveries`. Two independent, idempotent confirmation paths, both re-fetching the order from Razorpay's own API rather than trusting anything client-supplied:
  ...
  - Registered with Razorpay both in test mode (webhook id `TXsejV0SPg9oGJ`) and, as of 2026-09-05, in **live mode** (`TYPplPKed31qUF` — webhooks are scoped per mode, so going live required a second registration under the live key). Production is now running on live Razorpay credentials, not test ones.
...
- **International payment acceptance** — **approved and live** as of 2026-09-05. Was a self-serve dashboard request (Payment Methods → International Payments → Request), needing the standard website policy pages first (About/Contact/Shipping/Refund/Terms/Privacy — added, see below). Verified working end-to-end against the live API with a real (unpaid) USD order — see `PROGRESS.md`.
...
Razorpay is live (test and live modes both wired up, international payments approved) — a real end-to-end purchase (browser checkout → live card capture → webhook) hasn't been run yet and is worth doing once. Otherwise, Milestone 7 (comment-to-DM automation) is next. Dodo and Paddle are paused, not next. See `PROGRESS.md` for the full detail.
