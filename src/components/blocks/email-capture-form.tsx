...
export type EmailCaptureResult = { ok: true } | { ok: false; error: string };

/**
 * The email signup: a headline and one pill holding the field and the
 * button. Where the email goes is up to the caller, so the same form serves
 * a creator's email block and OrangeLink's own waitlists.
 */
export function EmailCaptureForm({
  ...
  ctaText: string;
  doneText?: string;
  onSubmit: (email: string) => Promise<EmailCaptureResult>;
}) {
    ...
    <div className="rounded-[28px] bg-surface-2 p-5">
      ...
      {status === "done" ? (
        ...
        >
          ...
          <p className="text-body">{doneText}</p>
        </motion.div>
      ) : (
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <p className="text-center text-[14px] font-semibold tracking-[-0.006em]">{headline}</p>
